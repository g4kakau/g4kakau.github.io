...
if rg -n '/tutoring-plans/' _tabs _includes index.html _data --glob '!tutoring.md'; then
  echo "ERROR: /tutoring-plans/ remains a primary navigation destination" >&2
  failures=1
fi

if [[ ! -f _tabs/academy.md ]] || ! rg -q '^permalink: /academy/$' _tabs/academy.md; then
  echo "ERROR: /academy/ bridge page is missing" >&2
  failures=1
fi

if [[ ! -f tutoring-plans/index.html ]] || ! rg -q "canonical.*academy" tutoring-plans/index.html; then
  echo "ERROR: legacy /tutoring-plans/ compatibility redirect is missing" >&2
  failures=1
fi

if [[ $(rg -l '<div class="cta-box">|\{% include cta\.html %\}' _posts | wc -l | tr -d ' ') != "0" ]]; then
