...
疊加論證需要的條件是：晶格頂點遞移（每個節點地位相同），**而且**每個節點的 $z$ 條鍵在對稱操作下彼此等價——後者才是「電流均分成 $z$ 份」的依據。（只有頂點遞移並不夠：例如在正方晶格上再加對角鍵，每個節點仍然地位相同，但軸向鍵與對角鍵不等價，電流不會均分。）滿足這兩個條件時，設配位數為 $z$，情境一和情境二各貢獻電壓降 $IR/z$，疊加後：
