...
categories: [大學數學, 代數]
tags: [等效電阻, 電阻網路, 對稱性, 等效節點, 節點電壓法, 電路分析, 競賽物理, 對稱群, 軌道, 立方晶格]
math: true
description: "為什麼立方體問題裡的某些節點電位一定相同？答案不是「感覺對稱」，而是可以從唯一性定理嚴格證明。本文介紹對稱操作如何把節點分成軌道，把 6 個內部未知電位壓縮成 2 個。立方晶格等效電阻系列第三篇。"
...
只要固定所有外部接點的電位（例如起點 $V_A = 1$，終點 $V_H = 0$），聯立方程式

$$\sum_{j \sim i} \frac{V_j - V_i}{R_{ij}} = 0 \quad \text{（對所有內部節點 }i\text{）} \tag{1}\label{eq3:kcl}$$

有且只有一組解。（這是線性聯立方程式，係數矩陣正定，因此解唯一。此即[第二篇式 (3)](/posts/resistor-cube-2-kirchhoff/)的節點電壓方程式。）
...
我們要找的是**固定 $A$ 和 $H$、同時保持電路結構的置換**。在這個電路中，每條邊對應一對座標差為 1 的節點，而且所有電阻值相同，所以任何保持「兩節點相鄰」關係的置換都是對稱操作。

關鍵觀察：三個座標軸 $x, y, z$ 的地位完全對等。任何**對 $\{x, y, z\}$ 的置換**都是一個對稱操作：
...
不用對稱性，一般方法要對 6 個內部節點各列一條 KCL，得到一個 $6\times 6$ 的線性聯立方程式。
...
4. **軌道 = 未知數**：聯立方程式的規模等於軌道數（減去邊界節點數）。

這個邏輯不只適用於電路——凡是有唯一解的物理問題，對稱性都能縮小求解規模。在量子力學裡，同樣的邏輯讓我們能用群的**不可約表示**（irreducible representation）來分類能態；在固態物理裡，它讓 Bloch 定理成立。若想深入了解這套邏輯的完整數學框架，可參考 Arfken, Weber & Harris 的 *Mathematical Methods for Physicists*（第七版）第 16 章，或 Tinkham 的 *Group Theory and Quantum Mechanics* 第 2–3 章。
