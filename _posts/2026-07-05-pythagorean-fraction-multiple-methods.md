...
因為 $a^2+b^2=c^2$，很自然聯想到直角三角形：把 $a,b$ 想成直角邊，$c$ 想成斜邊。設直角三角形中對應 $a$ 的銳角為 $2\theta$（**刻意寫成 $2\theta$ 而不是 $\theta$**，等一下會看到這讓兩個分式同時化簡）：

$$
a=c\sin2\theta,\qquad b=c\cos2\theta,\qquad 0<2\theta<90^\circ
$$

第一項用半角關係 $\dfrac{\sin2\theta}{1+\cos2\theta}=\tan\theta$ 立刻化成 $\tan\theta$——這正是取 $2\theta$ 的理由。
...
**分式方程式，先合併再化簡**。方法一之所以計算量大，是因為從一開始就把兩個分式分開處理；方法二、三都先把分式合併成一個，讓 $a^2+b^2=c^2$ 這個條件能一次性代入化簡。

**答案不唯一，是對稱條件的必然結果**，不是計算失誤。任何時候看到條件對兩個變數對稱，都要記得檢查：題目有沒有額外限定大小關係？沒有的話，答案通常成雙出現。
...
| 解法 | 關鍵技巧 | 計算量 | 優點 |
|---|---|---|---|
| 方法一：三角代換 | 設 $a=c\sin2\theta,b=c\cos2\theta$，用半角與二倍角公式 | 較大 | 直覺對應直角三角形，適合熟悉三角恆等式的人 |
