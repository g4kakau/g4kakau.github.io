...
categories: [大學物理, 電路學]
tags: [等效電阻, 電阻網路, 節點電壓法, 克希荷夫定律, 電路分析, 競賽物理, 立方晶格, 對稱性]
math: true
mermaid: true
description: "一個正方體的 12 條邊上各有一顆電阻 R，體對角線兩端的等效電阻是多少？從節點電壓法出發，用對稱性把 6 個內部未知電位壓縮成 2 個，列 KCL 方程式得出 5R/6。立方晶格等效電阻系列第一篇。"
...
把正方體放進座標系，每個頂點用三個 0 或 1 的座標表示。8 個頂點是 $(x, y, z)$，其中 $x, y, z \in \\{0, 1\\}$。
...
- **B 型節點**：$(1,0,0)$、$(0,1,0)$、$(0,0,1)$——三個節點在幾何上完全等價，與 A 的距離相同，連接方式相同。把 $x, y, z$ 座標任意交換，這三個節點會互相映射，卻不影響電路結構。所以它們的電位必然相等，記為 $b$。
...
>
> 對稱性把 6 個內部未知電位壓縮成 2 個，只需解 $2\times 2$ 線性聯立方程式即可得出。
{: .prompt-tip }
