...
**關鍵**：$I$ 在兩邊都出現，直接解代數方程式就能得到答案。這種「繞一圈回來」的情形稱為**循環積分**，只有在特定的函數組合（指數 $\times$ 三角）才會發生。
...
這個遞迴公式把 $n$ 次降成 $n-2$ 次，最終會歸結到 $I_1=\int\cos x\,\dd{x}=\sin x+C$（$n$ 為奇數）或 $I_0=\int\dd{x}=x+C$（$n$ 為偶數）。
