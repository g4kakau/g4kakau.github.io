...
電容與電感之間的對稱性，是電路理論裡最漂亮的結構之一。這不只是符號上的對應，而是來自電場與磁場在 Maxwell 聯立方程式中的深層關聯：在相對論框架下，電場與磁場是同一個電磁場張量的不同分量。只是現實世界有電荷、沒有觀測到磁單極，所以這種對稱在完整電磁學裡不是毫無條件的完美鏡像；在集總電路近似下，它才呈現成電容與電感這組漂亮的對偶。
...
>
> **① 電容與電感的對稱之美（本篇）**
> ② [電容與 RC 電路：水桶、充放電與時間常數](/posts/capacitor-and-rc-circuit/)  
