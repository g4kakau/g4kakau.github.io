...
要描述線圈「鏈住多少磁場」，需要引入**磁通鏈**（flux linkage）$\lambda$。對 $N$ 匝線圈，若每匝鏈住的磁通量近似為 $\Phi$，則：

<!-- 圖像需求：做一張靜態圖解磁通鏈的圖。畫 N 匝線圈穿過同一束磁場線，標示單匝磁通量 Phi 與總磁通鏈 lambda = N Phi。圖中要強調「不是磁場有多少，而是線圈實際鏈住多少磁通」。可加入一個對照小圖：磁場線沒有穿過線圈面時，鏈住的磁通較小。 -->

$$\lambda = N\Phi = N \int_S \vb{B} \cdot \dd{\vb{A}}$$
...
<!-- 互動圖需求：做一個「電容/電感對偶滑桿」或切換式互動圖。使用者可在電容與電感之間切換，畫面同步替換狀態變數 v/i、基本關係 q=Cv/lambda=Li、動態方程式 i=C dv/dt / v=L di/dt、不能突變量、DC 與高頻極限。目標是把下方表格變成可操作的記憶卡，適合嵌入文章。 -->
