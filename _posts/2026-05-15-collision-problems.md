...
math: true
description: "碰撞問題是高中物理的難題重點。本文系統整理彈性碰撞、完全非彈性碰撞的解題架構，推導一維彈性碰撞的速度公式，說明二維碰撞的分量列式方法，附質心座標系分析，幫你建立完整的解題流程。"
---
...
所以第二顆球必須偏向 $x$ 軸**下方** $60^\circ$（即上式中的 $\theta=60^\circ$，若以帶號角度表示則是 $-60^\circ$）——注意兩球必須分居 $x$ 軸兩側，否則 $y$ 方向動量無法相消。

代入動能守恆和一個動量分量，可得漂亮的結果：

$$v_1'=v_0\cos30^\circ=\frac{\sqrt3}{2}v_0,\qquad v_2'=v_0\sin30^\circ=\frac12 v_0$$

驗算：$v_1'^2+v_2'^2=\tfrac34v_0^2+\tfrac14v_0^2=v_0^2$ ✓，動能守恆成立。

---

## 質心座標系分析：讓碰撞問題更簡單

在**質心座標系**（center of mass frame）中，系統的質心靜止，兩物體的動量大小相等方向相反。

質心速度：

$$v_\text{cm} = \frac{m_1 v_1 + m_2 v_2}{m_1 + m_2}$$

在質心座標系中，彈性碰撞後每個物體的速度大小不變，只有方向反轉（一維情況）。這讓分析非常乾淨——計算在質心座標系做，最後再加回質心速度轉換回實驗室系。
