...
一組**正交基底**（orthogonal basis）有一個決定性的優點：在正交基底下，任何向量的座標都可以用**內積直接求出**，不需要解方程組。
...
把 $n$ 個線性獨立的列向量排成矩陣 $A = [\vb{v}_1 \mid \vb{v}_2 \mid \cdots \mid \vb{v}_n]$。
...
其中 $Q$ 的列是正規化後的 $\vb{e}_1, \ldots, \vb{e}_n$（標準正交矩陣），$R$ 是上三角矩陣（記錄每一步扣掉的投影係數）。
