...
categories: [大學物理, 力學]
tags: [拉格朗日力學, 分析力學, 廣義座標, 歐拉－拉格朗日方程式, 單擺, 大學物理]
math: true
description: "牛頓力學用力向量描述運動；拉格朗日力學改用能量差。本文以單擺為例，介紹廣義座標的概念，推導歐拉－拉格朗日方程式，並說明為何張力（約束力）在這個框架中自動消失。"
---
...
> 用「最少數量的座標」描述系統，讓約束自動內建進去，再對「動能 − 位能」做變分，就能得到運動方程式，約束力自然不出現。

---

## 廣義座標

考慮一個有 $n$ 個自由度的系統。**廣義座標**（generalized coordinates）$q_1, q_2, \ldots, q_n$ 是描述系統所需的最少獨立變數，不一定是 $x, y, z$。

**單擺的例子：** 擺錘在平面上運動，受繩子約束在半徑 $L$ 的圓弧上。$x$ 和 $y$ 都不是獨立的（因為 $x^2 + y^2 = L^2$），只需*一個廣義座標*：
...
對每個廣義座標 $q_i$，運動方程式由 **歐拉－拉格朗日方程式（Euler–Lagrange equation，簡稱 E-L 方程式）**給出：
...
和第一篇的牛頓法結果完全一致——但整個推導中**從未出現繩子的張力**。張力是約束力，不對廣義座標 $\theta$ 做功，在拉格朗日框架中自然隱去。

---

## 廣義動量與守恆律

對應廣義座標 $q_i$ 的**廣義動量**（generalized momentum）定義為：
...
如果 $\mathcal{L}$ 不顯含某個廣義座標 $q_i$，就稱為**循環座標**（cyclic coordinate），則 $\partial\mathcal{L}/\partial q_i = 0$，對應的廣義動量守恆：
...
- 廣義座標 $q_i$：描述系統最少的獨立自由度，約束自動內建
- 拉格朗日量 $\mathcal{L} = T - V$：用純量能量描述動力學
- E–L 方程式：每個廣義座標一條，見式 $\eqref{eq:euler-lagrange}$
- 廣義動量：循環座標對應守恆量，定義見式 $\eqref{eq:generalized-momentum}$
