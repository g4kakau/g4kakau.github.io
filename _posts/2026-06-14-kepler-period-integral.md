...
更有趣的是：橢圓軌道的周長本身需要[第二類橢圓積分 $E(k)$](/posts/elliptic-integral-E/)才能算出來，[單擺的週期](/posts/pendulum-not-shm/)也需要[第一類橢圓積分 $K(k)$](/posts/elliptic-integral-K/)。那麼克卜勒軌道的週期呢？答案會讓你意外——**完全不需要橢圓積分**。這背後的原因，正是這篇文章的重點。
...
其中 $W(q)=\displaystyle\int\sqrt{2m(E-V(q))}\,dq$。Hamilton-Jacobi 方程的一個核心結果是：時間與能量的關係由
...
[在系列③](/posts/effective-potential-orbits/)中，我們已經把二維軌道問題化為一維的有效位能問題：
...
$$
dt=\frac{dr}{\dot r}=\frac{dr}{\sqrt{2(\varepsilon-V_{\text{eff}}(r))}}
$$

這正是 $t-t_0=\partial W/\partial\varepsilon$ 的具體寫法（$W=\int\sqrt{2(\varepsilon-V_{\text{eff}})}\,dr$，對 $\varepsilon$ 微分後被積函數恰好是 $1/\sqrt{2(\varepsilon-V_{\text{eff}})}$）。

**週期**就是 $r$ 從近日點 $r_{\min}$ 走到遠日點 $r_{\max}$ 再走回來的時間，由對稱性，等於這段單程時間的兩倍：

$$
T=2\int_{r_{\min}}^{r_{\max}}\frac{dr}{\sqrt{2(\varepsilon-V_{\text{eff}}(r))}}
$$
...
[由系列④](/posts/kepler-energy-angular-momentum/)，能量與角動量可以用半長軸 $a$ 與離心率 $e$ 表示：
...
這個式子很值得停下來看一眼：$a^2e^2-(r-a)^2\geq0$ 正好給出 $r\in[a(1-e),a(1+e)]=[r_{\min},r_{\max}]$——這就是橢圓軌道 $r$ 的允許範圍，和[系列③的轉折點分析](/posts/effective-potential-orbits/)完全一致。而 $[a^2e^2-(r-a)^2]$ 這個結構，是「圓」的方程式 $a^2e^2=(r-a)^2+(\cdots)^2$ 的影子——這就是接下來代換的線索。
...
$$
dt=\frac{dr}{\dot r}=\frac{ae\sin\psi\,d\psi\cdot r}{e\sqrt{GMa}\sin\psi}=\sqrt{\frac{a}{GM}}\,r\,d\psi=\sqrt{\frac{a^3}{GM}}(1-e\cos\psi)\,d\psi
$$

**這一步是整個推導的關鍵**：$\sin\psi$ 完全消失了，剩下的 $(1-e\cos\psi)\,d\psi$ 是一個**初等函數**，可以直接積分。
...
$$
t=\sqrt{\frac{a^3}{GM}}\int_0^\psi(1-e\cos\psi')\,d\psi'=\sqrt{\frac{a^3}{GM}}\left(\psi-e\sin\psi\right)
$$
...
[在橢圓積分系列](/posts/elliptic-integral-K/)，[單擺的週期](/posts/pendulum-not-shm/)積分是

$$
T=4\sqrt{\frac{l}{g}}\int_0^{\pi/2}\frac{d\phi}{\sqrt{1-k^2\sin^2\phi}}=4\sqrt{\frac{l}{g}}K(k)
$$

這個積分**沒有初等反導函數**——$K(k)$ 是一個獨立定義的特殊函數。而[橢圓周長](/posts/elliptic-integral-E/)的積分 $E(k)$ 也是同樣的故事。
...
這不是偶然。萬有引力 $\propto 1/r^2$（也就是 $V\propto1/r$）是少數幾種能讓所有**有界軌道都是封閉曲線**的中心力場之一（另一個是簡諧振子 $V\propto r^2$）。這種特殊性背後，藏著一個額外的守恆量——**Laplace–Runge–Lenz 向量**，它讓克卜勒問題擁有比一般中心力問題更多的對稱性（數學上稱為「超可積」）。下一篇會正式介紹這個向量，以及它如何解釋橢圓軌道為什麼不會像[單擺的相圖](/posts/phase-space-pendulum/)那樣需要橢圓積分。
...
| 量 | 結果 | 是否初等 |
|---|---|---|
| 週期積分 $T=2\displaystyle\int_{r_{\min}}^{r_{\max}}\dfrac{dr}{\sqrt{2(\varepsilon-V_{\text{eff}})}}$ | 經離心近點角代換後 $=2\pi\sqrt{a^3/GM}$ | ✅ 初等（克卜勒第三定律） |
