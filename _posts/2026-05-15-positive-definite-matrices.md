...
$$
D_3 = 2(4-1) - (-1)\bigl(-2-(-1)b\bigr) + b(1-2b)
= 6 + (b-2) + b - 2b^2
= 4 + 2b - 2b^2
$$
...
**注意：** $X=V\sqrt\Lambda$ 只是眾多可能的分解之一——對任意正交矩陣 $U$，$XU$ 也滿足 $(XU)(XU)^T=XUU^TX^T=Q$，所以 $Q=XX^T$ 這個寫法**本身並不唯一**。在所有這些 $X$ 之中，恰好有一個可以取成「下三角且對角線全正」的形式，那才是唯一的 Cholesky 分解 $Q=LL^T$。若有零特徵值（半正定），$Q$ 不可逆，分解仍然存在，但連下三角的取法也不再唯一。
