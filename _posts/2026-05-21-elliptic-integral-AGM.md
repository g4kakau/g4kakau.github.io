---
layout: post
title: "橢圓積分怎麼算：AGM 演算法與冪級數展開"
date: 2026-05-21 02:00:00 +0800
categories: [大學數學, 微積分]
tags: [橢圓積分, AGM, 算術幾何平均, 數值方法, 特殊函數, 大學微積分]
math: true
description: "第一類橢圓積分 K(k) 沒有初等函數公式，但可以高效率地計算。本文介紹兩種方法：冪級數展開（適合小 k）與算術幾何平均（AGM）演算法。AGM 每迭代一次有效位數倍增，只需約 10 步即可達到機器精度。"
---

$$\require{physics}$$
<link rel="stylesheet" href="/assets/css/posts-custom.css">

[上一篇]({% post_url 2026-05-21-elliptic-integral-K %})說明了 $K(k)$ 無法用初等函數表示。但「無法用初等函數表示」不代表算不出來——只是需要適當的演算法。

本篇介紹兩種計算 $K(k)$ 的方法：

1. **冪級數展開**：直接、透明，適合 $k$ 不太大時
2. **算術幾何平均（AGM）演算法**：收斂極快，任意精度首選
...
對 $k = 0.5$，取到 $k^6$ 項（$N=3$）的相對誤差約 $3\times10^{-4}$，也就是三、四位有效數字；要到 $10^{-6}$ 需要 $7$ 項。但 $k \to 1$ 時收斂極慢（因為 $K(k)$ 本身趨向無限大），需要對數修正或其他方法。

---

## 方法二：算術幾何平均（AGM）演算法

這個方法由高斯發現，是計算 $K(k)$ 最優雅也最有效率的方式。
...
相比之下，冪級數在 $k = 0.9$ 時要約 $47$ 項才有 $10^{-6}$ 的相對精度，要達到雙精度更需要約 $140$ 項。
...
= (1+k_1)\int_0^{\pi/2}\frac{\dd{\psi}}{\sqrt{1 - k_1^2\sin^2\psi}}
\qquad\text{即}\qquad K(k)=(1+k_1)K(k_1)$$

其中 $k_1 = (1 - k')/(1 + k')$ 是一次 AGM 迭代後的新模數（這是 Landen 降序變換）。注意兩個積分**不相等**：因為 $k_1<k$ 而 $K$ 遞增，必須有 $(1+k_1)$ 這個因子才能配平。反覆應用這個恆等式，模數序列收斂到 $0$，此時 $K(0)=\pi/2$，而一路累積下來的 $(1+k_1)(1+k_2)\cdots$ 正好給出 $1/\text{AGM}(1,k')$。
...
| $n$ | $a_n$ | $b_n$ | $a_n - b_n$ |
|:---:|:---:|:---:|:---:|
| 0 | $1.000000$ | $0.707107$ | $2.9\times10^{-1}$ |
| 1 | $0.853553$ | $0.840896$ | $1.3\times10^{-2}$ |
| 2 | $0.847225$ | $0.847201$ | $2.4\times10^{-5}$ |
| 3 | $0.847213$ | $0.847213$ | $8.2\times10^{-11}$ |

**表 1：** $\text{AGM}(1, 1/\sqrt{2})$ 的迭代過程。誤差依 $\varepsilon_{n+1}\approx\varepsilon_n^2/(8a)$ 平方遞減（$10^{-1}\to10^{-2}\to10^{-5}\to10^{-11}$），三步就到約 10 位精度，第四步即達雙精度極限。
{: .fig-caption }
...
$$K\!\left(\frac{1}{\sqrt{2}}\right) = \frac{\pi}{2 \times 0.847213} \approx 1.8541$$
...
k = 0.90 | AGM = 2.28054914 | 冪級數 = 2.27928028  ← 第四位就開始差
k = 0.99 | AGM = 3.35660052 | 冪級數 = 3.01766890  ← 差了 10%
```

注意：$k = 0.99$ 時冪級數取 $20$ 項還差了 $10\%$（要達到 $10^{-6}$ 的相對精度需要約 $480$ 項），而 AGM 四、五步就到機器精度。

`scipy.special.ellipk(m)` 也是用 AGM 類演算法實作的（注意：`scipy` 的參數是 $m = k^2$，不是 $k$ 本身）。
...
$K(k)$ 雖然沒有初等函數公式，但可以高效率地計算：

- **冪級數**：$K(k) = \frac{\pi}{2}\left[1 + k^2/4 + 9k^4/64 + \cdots\right]$，小 $k$ 時快速收斂
- **AGM 演算法**：$K(k) = \pi / (2\,\text{AGM}(1, \sqrt{1-k^2}))$，平方收斂，約 10 步達雙精度
