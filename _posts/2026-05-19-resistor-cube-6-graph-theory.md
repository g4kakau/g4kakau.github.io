...
把正方體 8 個頂點的三維坐標 $(x,y,z)$ 直接編碼成整數：節點 $k$ 的坐標為 $k$ 的二進位表示 $(k_2, k_1, k_0)$，其中 $k = 4k_2 + 2k_1 + k_0$。

| 節點 $k$ | 二進位 | 坐標 $(x,y,z)$ | 系列前五篇的稱呼 |
| :------: | :----: | -------------- | ---------------- |
|    0     |  000   | $(0,0,0)$      | 起點 $A$         |
|    1     |  001   | $(0,0,1)$      | —                |
|    2     |  010   | $(0,1,0)$      | —                |
...
[第三篇](/posts/resistor-cube-3-symmetry/)和[第五篇](/posts/resistor-cube-5-three-configurations/)用對稱群 $S_3$ 把節點分成軌道，等價於說：對應的坐標置換 $\sigma$ 作用在節點編號上，會把 $\tilde{L}$ 的某些列（行）互換，而 $\tilde{L}$ 保持不變——即 $P_\sigma \tilde{L} P_\sigma^\top = \tilde{L}$，其中 $P_\sigma$ 是置換矩陣。
