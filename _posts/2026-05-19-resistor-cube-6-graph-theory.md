...
<!-- VISUAL: 做一張三角形電路與其鄰接矩陣的對照圖。左邊是三個節點 $1,2,3$ 兩兩相連、每條邊標 $R$ 或 $G$；右邊是 $3\times3$ 鄰接矩陣，三個非對角的 $G$ 用邊的顏色對應，高亮對角線為 0，讓讀者看出「沒有自迴路」。 -->
...
<!-- VISUAL: 做一張 $\tilde{L}$ 矩陣熱圖或互動矩陣。對角線的 3 用同一色標出；每個 -1 點擊或 hover 時，同步高亮正方體上的對應邊；0 則可提示「不相鄰」。特別預設高亮 $\tilde{L}_{07}=0$ 與 $\tilde{L}_{37}=-1$，對應下方文字驗證。 -->
